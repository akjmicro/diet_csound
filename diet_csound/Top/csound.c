...
void print_csound_version(CSOUND* csound)
{
    ...
    csoundErrorMsg(csound,
                    Str("--Diet Csound version %s beta (double samples) %s\n"
                        "[commit: %s]\n"),
                    CS_PACKAGE_VERSION, CS_PACKAGE_DATE,
                    STRING_HASH(GIT_HASH_VALUE));
#else
    csoundErrorMsg(csound, Str("--Diet Csound version %s (double samples) %s\n"
                                "[commit: %s]\n"),
                    CS_PACKAGE_VERSION, CS_PACKAGE_DATE
                    , STRING_HASH(GIT_HASH_VALUE));
#endif
#else
#ifdef BETA
    csoundErrorMsg(csound, Str("--Diet Csound version %s beta (float samples) %s\n"
                                "[commit: %s]\n"),
                    CS_PACKAGE_VERSION, CS_PACKAGE_DATE,
                    STRING_HASH(GIT_HASH_VALUE));
#else
    csoundErrorMsg(csound, Str("--Diet Csound version %s (float samples) %s\n"
                                "[commit: %s]\n"),
                    CS_PACKAGE_VERSION, CS_PACKAGE_DATE,
                    STRING_HASH(GIT_HASH_VALUE));
#endif
